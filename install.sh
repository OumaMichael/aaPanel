...
Install_sqlite3_for_nginx()
{
    ...
    if [ true ];then
		...
		echo 'Compiling the lsqlite3 module'
        ...
        if [ -f '/www/server/panel/plugin/btwaf/lsqlite3_fsl09y/lsqlite3.so' ];then
			...
			echo "Failed to extract sqlite3"
        fi
...
bt_syssafe_stop() {
	if [ -f "/etc/init.d/bt_syssafe" ]; then
		...
		if [[ $status == *"already running"* ]]; then
			echo 'System hardening is temporarily shutting down'
...
bt_syssafe_start() {
	if [ -f "/tmp/.bt_syssafe_stop" ]; then
		...
		if [[ $status == *"not running"* ]]; then
			echo 'System reinforcement restart'
...
{	
	...
	if [ -f "/etc/init.d/btwaf" ]; then
		...
		bt_syssafe_start
		echo 'If the installation fails, please disable system hardening and try again, or other security software such as software'
		echo 'The installation process ends after 5 seconds！！！！'
		sleep 5
		echo 'The installation failed, and there may be some security software that caused the firewall scheduled task to fail to start' > $install_tmp
		exit 1
	fi 
}
...
{ 
	...
	# 线上用 aapanel静态资源下载地址
  	wget --no-check-certificate -O $pluginPath/btwaf_static.zip  $download_Url/install/plugin/btwaf_en/btwaf_static2.zip  -T 5
# 	wget --no-check-certificate -O $pluginPath/btwaf_static.zip  $download_Url/install/plugin/btwaf_en/btwaf_static.zip -T 5
	# 测试用
#	wget -O $pluginPath/btwaf_static.zip  http://download.bt.cn/btwaf_rule/btwaf_static2.zip
	sleep 0.5
	...
	echo "Size of the downloaded resource package:"$count;
	if [ $count -lt 20038116 ];then
		echo 'Failed to download the resource package' > $install_tmp
		echo 'ERROR: The installation failed' > $install_tmp
		echo 'If the installation fails, you can reinstall it again'
		echo 'The installation process ends after 5 seconds!'
		sleep 5
		echo 'The installation failed, and the download of the resource package failed, please try again' > $install_tmp
		exit 1
	fi 
	...
	if [ -d $pluginPath/btwaf_static ];then
		echo "The decompression was successful"
	else
		echo 'Failed to decompress' > $install_tmp
		echo 'ERROR: The installation failed' > $install_tmp
	fi
  ...
  # 换英文ip库
	mv -f  $pluginPath/GeoLite2-City.mmdb /www/server/btwaf/inc/GeoLite2-City.mmdb
	...
	if [ ! -f $btwaf_path/html/get.html ];then
		...
		\cp -a -r $pluginPath/btwaf_lua/html/post.html $btwaf_path/html/post.html
		\cp -a -r $pluginPath/btwaf_lua/html/cookie.html $btwaf_path/html/cookie.html
		\cp -a -r $pluginPath/btwaf_lua/html/user_agent.html $btwaf_path/html/user_agent.html
		\cp -a -r $pluginPath/btwaf_lua/html/other.html $btwaf_path/html/other.html
	fi
	...
	\cp -a -r $pluginPath/btwaf_lua/rule/get.json $btwaf_path/rule/get.json
	\cp -a -r $pluginPath/btwaf_lua/rule/other_rule.json $btwaf_path/rule/other_rule.json
	...
	\cp -a -r $pluginPath/btwaf_lua/inc/bt_engine_arm.so $btwaf_path/inc/bt_engine_arm.so
	...
	\cp -a -r $pluginPath/btwaf_lua/public $btwaf_path/


	if [ -d '/www/server/btwaf/inc/cms' ];then
		chattr -R  -ia /www/server/btwaf/inc/cms
		rm -rf  /www/server/btwaf/inc/cms
	fi
	...
	if [ ! -f /www/server/panel/vhost/nginx/btwaf.conf ];then
		...
		echo 'The installation failed and the configuration file does not exist'
		echo 'The installation failed, it is recommended to reinstall it again'
		echo 'The installation process ends after 5 seconds！！！！'
		sleep 5
		echo 'The installation failed and the configuration file does not exist' > $install_tmp
		exit 1
	fi
	...
	if [ ! -f /www/server/panel/vhost/nginx/0.btwaf_log_format.conf ];then
		rm -rf /www/server/panel/plugin/btwaf
		echo 'The installation failed, The configuration file [0.btwaf_log_format.conf] does not exist'
		echo 'The installation failed, it is recommended to reinstall it again'
		echo 'The installation process ends after 5 seconds！！！！'
		sleep 5
		echo 'The installation failed, The configuration file [0.btwaf_log_format.conf] does not exist' > $install_tmp
		exit 1
	fi
	...
	if [ ! -f /www/server/btwaf/inc/GeoLite2-City.mmdb ];then
		...
		/etc/init.d/nginx restart
		rm -rf /www/server/panel/plugin/btwaf
		echo 'The installation failed and the IP library file was missing'
		echo 'The installation failed, it is recommended to reinstall it again'
		echo 'The installation process ends after 5 seconds！！！！'
		sleep 5
		echo 'The installation failed, and there may be some security software that caused the installation of the IP library file to fail' > $install_tmp
		exit 1
	fi 
	...
	if [ ! -f /www/server/btwaf/inc/bt_engine.so ];then
		...
		/etc/init.d/nginx restart
		rm -rf /www/server/panel/plugin/btwaf
		echo 'The installation failed, and the installation of the semantic analysis module failed'
		echo 'If the installation fails, we recommend that you disable the cloud lock or other security software and install it again'
		echo 'The installation process ends after 5 seconds！！！！'
		sleep 5
		echo 'The installation failed, and the installation was blocked by a cloud lock or other security software' > $install_tmp
		exit 1
	fi 
	...
	if [ ! -f /www/server/btwaf/inc/lsqlite3.so ];then
		...
		/etc/init.d/nginx restart
		rm -rf /www/server/panel/plugin/btwaf
		echo 'The installation failed, and the sqlite 3 database module failed to install'
		echo 'If the installation fails, we recommend that you disable the cloud lock or other security software and install it again'
		echo 'The installation process ends after 5 seconds！！！！'
		sleep 5
		echo 'The installation failed, and the installation was blocked by a cloud lock or other security software' > $install_tmp
	...
	if [ ! -f /www/server/btwaf/inc/cjson.so ];then
		...
		/etc/init.d/nginx restart
		rm -rf /www/server/panel/plugin/btwaf
		echo 'The installation failed, and the cjson module was not installed successfully'
		echo 'If the installation fails, we recommend that you disable the cloud lock or other security software and install it again'
		echo 'The installation process ends after 5 seconds！！！！'
		sleep 5
		echo 'The installation failed, and the installation was blocked by a cloud lock or other security software' > $install_tmp
	...
	if [ ! -f /www/server/btwaf/inc/php_engine.so ];then
		...
		/etc/init.d/nginx restart
		rm -rf /www/server/panel/plugin/btwaf
		echo 'If the installation fails, there may be other security software such as Cloud Lock'
		echo 'If the installation fails, we recommend that you disable the cloud lock or other security software and install it again'
		echo 'The installation process ends after 5 seconds！！！！'
		sleep 5
		echo 'The installation failed, and the installation was blocked by a cloud lock or other security software' > $install_tmp
	...
	if [ $is_status -eq 0 ]; then 
		...
		rm -rf /www/server/panel/plugin/btwaf
		echo 'If the installation fails, you can reinstall it again'
		echo 'The installation fails, it may not be compatible with the Nginx, please switch to an Nginx version and reinstall it'
		echo 'The installation process ends in 5 seconds!'
		sleep 7
		echo 'The installation fails, it may not be compatible with the Nginx, please switch to an Nginx version and reinstall it' > $install_tmp
	else
